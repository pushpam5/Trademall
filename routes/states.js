...
router.get('/', function(req, res) {
  ...
  let  query =  States.find({country_id: req.query.country_id});
...
router.post('/', async (req, res) => {
    ...
    // if (error) {
    //     return res.status(400).send({success:0, message: error.details[0].context.label, error:1});
    // }
 
    // // Check if this cities already exisits
    // let cities = await Cities.findOne({ name: req.body.name, state_id: req.body.state_id });
    // if (cities) {
    // 	 return res.status(400).send({success:0, message: 'This city name is already exits!', error:1});
    // } else {
        // Insert the new cities if they do not exist yet
       ...
       for (var i=0; i<body.length; i++) {
       	  let states = new States({ state_id: body[i].id, name: body[i].name, country_id: body[i].country_id });
           await states.save();
       }
